	...
	*/
	...
	class GoogleGraph {
		...
		*/
		...
		protected function _extEncode($values, $max = 4095, $min = 0){
            $extended_table =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-.';
            ...
            foreach($values as $k => $v){
                    if($v >= $min && $v <= $max){
                            $first = substr($extended_table, round(($v - $min) / $size * $max /
    $delta),1);
		...
		public function printGraph() {
			...
			*/
			...
			if(!empty($this->Graph->axis_ranges)) {	
				$url .= 'chxr=';
				foreach($this->Graph->axis_ranges as $ranges)
					$url .= implode(',', $ranges).'|';
				$url = substr($url, 0, -1);			
				$url .= '&';		
			}
